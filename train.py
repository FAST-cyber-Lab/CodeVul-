...
def train_and_evaluate(dataset_path, save_path="pretrained/pretrained.pt", batch_size=8, epochs=3, learning_rate=1e-4, val_split=0.2):
    ...
    token_lengths = [len(embedder.tokenizer.encode(clean_code(code), truncation=False)) 
                    for code in train_codes[:min(100, len(train_codes))] + 
                    val_codes[:min(100, len(val_codes))]]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_path", type=str, default="pretrained/pretrained.pt", help="Path to save trained model")
